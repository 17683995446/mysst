...
# file = r'./2020/mhws_intensity_2020.01.nc'
...
# # 获取单独的一个变量的数据
# precipitationCal=dataset.variables['mhws_intensity'][:]
...
# data = dataset.variables['mhws_intensity'][1,:,:]
...
#     datai=dataset.variables['mhws_intensity'][:,340:380,1000-720:1200-720]
#     mhws.append(datai)
# mhws = np.concatenate(mhws)
# print(mhws.shape)
# dataset
# for i in range(0,len(mhws)-19):
#     dataset.append()
...
# for i in range(0,len(dataset.variables['mhws_intensity']),10):
#     print(i)
#     datai=dataset.variables['mhws_intensity'][i,340:380,1000-720:1200-720]#,62:222,306:530] #温度数据切片，选择第30个时间点的温度；将原温度转换为摄氏度
#     print(np.mean(datai))
#     print(np.min(datai))
#     print(np.max(datai))
...
def load_sst(root):
    ...
    for file_name in file_name_list[:-1]:
        ...
        datai = np.array(datai)
            ...
            # dataset_y.append(datai[i+10:i+20])
            
        mhws.append(datai)
    mhws = np.concatenate(mhws)
    print(mhws.shape,len(mhws))
    for i in range(0,len(mhws)-19):
        dataset_list.append(mhws[i:i+20])
    dataset = np.array(dataset_list)
    print("before:",dataset.shape)
    B,T,H,W = dataset.shape
    dataset = dataset.reshape(B,T,1,H,W)
    ...
    '''
    mhws = np.concatenate(mhws)
    print("before:",mhws.shape)
    T,H,W = mhws.shape
    mhws = mhws.reshape(T,1,H,W)
    mhws = (mhws - np.min(mhws))/(np.max(mhws) - np.min(mhws))
    print("after:",mhws.shape)
    dataset_x = []
    dataset_y = []
    for i in range(0,len(mhws)-19):
        dataset_x.append(mhws[i:i+10])
        dataset_y.append(mhws[i+10:i+20])
        # print(len(mhws[i:i+20]))
    '''
    ...
    train_x,train_y = X[:,:10],X[:,10:]
    test_x,test_y  = Y[:,:10],Y[:,10:]
    # train_x = np.array(dataset_x[:int(0.8*len(dataset_x))])
    # train_y = np.array(dataset_y[:int(0.8*len(dataset_y))])
    # test_x = np.array(dataset_x[int(0.8*len(dataset_x)):])
    # test_y = np.array(dataset_y[int(0.8*len(dataset_y)):])
